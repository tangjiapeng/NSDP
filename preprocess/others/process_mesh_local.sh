...
#
# we skip the sdf and surface point sampling of single frame, as we focus on temporal deformation 
# and the shape is given as the input, we don't need to predict the shape 
#
...
# The original mesh is no longer needed on disk; we have the transformation,
# so if we need it we can load the normalized mesh and transform it to the oringinal frame.
# rm $mesh_orig
